...
%% 数据开关
cfg.grid_size_m = 200;                  % 网格粒度主开关：1000、500、200 或 100 米。
...
cfg.use_processed_distance_matrix = false; % 是否使用 distance_matrix_* 文件覆盖服务网格间距离；100m 文件很大，默认关闭。
cfg.load_od_flow = false;                % 是否把 od_flow_grid_* 读入 mdl.data.od_flow；当前模型暂不需要，默认关闭。
cfg.load_raw_distance_matrix = false;    % 是否把 distance_matrix_* 全量读入 mdl.data.raw_distance_matrix；100m 下很大，默认关闭。
...
cfg.split_large_service_nodes = true;    % 是否将大短缺/大富余节点拆成多个访问副本，以允许同一真实网格被同一辆车多次访问。
cfg.node_split_threshold_bikes = NaN;    % 节点拆分阈值；NaN 表示使用调度车最小运载量。
cfg.node_split_unit_bikes = NaN;         % 每个副本分配的目标单车量；NaN 表示使用调度车最小运载量。
cfg.node_split_max_copies = inf;         % 每个真实网格最多拆分出的副本数；inf 表示不额外限制。
...
cfg.default_max_route_time_min = 360.0;       % 单辆车单次高峰调度最大工作时间，单位：分钟。
...
cfg.big_m_load = 1.0e4;                  % 保留参数；当前载重约束默认使用 M_k=2Q_k+max(s)+max(r)。
...
cfg.solver.scale_rows = true;            % 是否对约束行做尺度化；建议保持开启，可降低数值病态且开销较小。
cfg.solver.remove_dependent_equalities = false; % 是否用 QR 删除相关等式；更稳定但较慢，只有警告严重时再开启。
cfg.solver.max_dense_rank_rows = 300;     % QR 删除相关等式允许处理的最大等式行数，防止大模型反复 dense 化。
...
cfg.exact_validation.same_as_main_model = true; % 若为 true，直接验证 main 中同一个模型和同一个启发式解；若为 false，则构造小规模验证模型。
cfg.exact_validation.max_service_nodes = 30; % 小规模精确验证使用的服务节点数量；自写精确求解器建议先控制在 6-8 个节点。
cfg.exact_validation.max_vehicles_to_use = 6; % 小规模精确验证使用的车辆数量；车辆数增大会显著放大分支树。
cfg.exact_validation.max_cut_rounds = 5; % 最多动态添加几轮子回路割。
cfg.exact_validation.max_seconds = 120;  % 精确验证总时间上限，单位：秒。
cfg.exact_validation.max_branch_nodes = 50000; % 每轮内部 MILP 分支节点上限。
cfg.exact_validation.max_lp_rows = 100000; % 单个 LP 标准型最大约束行数，超过则停止验证，避免卡在基矩阵求解。
cfg.exact_validation.max_lp_nonzeros = 500000; % 单个 LP 标准型最大非零元数量，超过则停止验证。
cfg.exact_validation.objective_epsilon = 1.0e-6; % 搜索比当前启发式上界更优解时使用的严格改进量。
